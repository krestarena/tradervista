<div class="row gutters-16">
    ...
    @endphp
    @php
        $availableDispatchers = collect($dispatcherOptions ?? []);
    @endphp
    <!-- Product List -->
    <div class="{{ $col_val }}">
    ...
    <div class="col-md-6 mb-2">
            ...
            <div class="col-12 mt-3">
                <label class="aiz-megabox d-block bg-white mb-0">
                    <input
                        ...
                        onchange="show_pickup_point(this, {{ $owner_id }})"
                        data-target=".pickup_point_id_{{ $owner_id }}">
                    <span class="d-flex flex-column aiz-megabox-elem rounded-0" style="padding: 0.75rem 1.2rem;">
                        <span class="d-flex align-items-center">
                            <span class="aiz-rounded-check flex-shrink-0 mt-1"></span>
                            <span class="flex-grow-1 pl-3 fw-600">{{ translate('Use my own dispatch') }}</span>
                        </span>
                        <small class="text-muted pl-5">{{ translate('Immediate vendor payout applies once you confirm the disclaimer.') }}</small>
                    </span>
                </label>
            </div>
            @endif
        ...
        @endif
        @if ($availableDispatchers->isNotEmpty())

        <div class="row pt-3 dispatcher_id_{{ $owner_id }} d-none">
            @foreach($availableDispatchers as $dispatcher)
            <div class="col-md-12 mb-2">
                <label class="aiz-megabox d-block bg-white mb-0">
                    <input
                        type="radio"
                        name="dispatcher_id_{{ $owner_id }}"
                        value="{{ $dispatcher['id'] }}"
                        @if($loop->first) checked @endif
                        onchange="updateDeliveryInfo('dispatcher', {{ $dispatcher['id'] }}, {{ $owner_id }})">
                    <span class="d-flex flex-wrap p-3 aiz-megabox-elem rounded-0">
                        <span class="aiz-rounded-check flex-shrink-0 mt-1"></span>
                        <span class="flex-grow-1 pl-3 fw-700">{{ $dispatcher['name'] }}</span>
                        <span class="flex-grow-1 pl-3 fw-600">{{ translate('ETA: :hours hrs', ['hours' => $dispatcher['eta_hours']]) }}</span>
                        <span class="flex-grow-1 pl-4 pl-sm-3 fw-600 mt-2 mt-sm-0 text-sm-right">{{ single_price($dispatcher['rate']) }}</span>
                    </span>
                </label>
            </div>
            @endforeach
