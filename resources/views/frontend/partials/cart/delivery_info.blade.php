...
@if (!empty($admin_products))
    <div class="card mb-3 border-left-0 border-top-0 border-right-0 border-bottom rounded-0 shadow-none">
        ...
        <div class="card-body p-0">
            @include('frontend.partials.cart.delivery_info_details', ['products' => $admin_products, 'product_variation' => $admin_product_variation, 'owner_id' => get_admin()->id, 'dispatcherOptions' => $dispatcherOptions ?? collect() ])
        </div>
    </div>
@endif
...
@if (!empty($seller_products))
    @foreach ($seller_products as $key => $seller_product)
        <div class="card @if($loop->last) mb-0 @else mb-3 @endif border-left-0 border-top-0 border-right-0 @if($loop->last) border-bottom-0 @else border-bottom @endif rounded-0 shadow-none">
            ...
            <div class="card-body p-0">
                @include('frontend.partials.cart.delivery_info_details', ['products' => $seller_product, 'product_variation' => $seller_product_variation, 'owner_id' => $key, 'dispatcherOptions' => $dispatcherOptions ?? collect() ])
            </div>
        </div>
    @endforeach
@endif
