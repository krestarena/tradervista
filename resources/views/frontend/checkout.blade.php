                            ...
                            <div class="card rounded-0 mb-0 border shadow-none">
                                ...
                                <div id="collapsePaymentInfo" class="collapse show" aria-labelledby="headingPaymentInfo" data-parent="#accordioncCheckoutInfo">
                                    <div class="card-body" id="payment_info">
                                        @include('frontend.partials.cart.payment_info', ['carts' => $carts, 'total' => $total, 'voucherContext' => $voucherContext ?? null])
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content rounded-0">
                    <div class="modal-header border-0">
                        <h5 class="modal-title">{{ translate('Important Notice') }}</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <p class="mb-3">{{ translate('You have chosen to use your own dispatch. TradeVista will not be responsible for delivery failures. Payment will be released to the vendor immediately.') }}</p>
                        <p class="text-muted small mb-0">{{ translate('Confirm to proceed with immediate vendor payout for this seller.') }}</p>
                    </div>
                    <div class="modal-footer border-0">
                        <button type="button" class="btn btn-link" id="ownDispatchDecline" data-dismiss="modal">{{ translate('Cancel') }}</button>
                        <button type="button" class="btn btn-primary" id="ownDispatchConfirm">{{ translate('I understand & continue') }}</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script type="text/javascript">
       var carrierCount=0;
            ...
            if (content.length > 0) {
                ...
                if (content_checked.length > 0) {
                    content_checked.each(function (i, el) {
                        ...
                        }else if($(el).val() == 'dispatcher'){
                            var owner = $(el).attr('data-owner');
                            if ($('input[name=dispatcher_id_'+owner+']:checked').length > 0) {
                                allOk = true;
                            }
                        }else{
                            allOk = true;
        ...
        function show_pickup_point(el, user_id) {
            ...
            if (type === 'own_dispatch' && tradevistaOwnDispatchModalEnabled) {
                if (!confirmedOwnDispatchSellers[user_id]) {
                    pendingOwnDispatchSelection = { el: el, user_id: user_id };
                    $('#ownDispatchModal').modal('show');
                    return;
                }
            } else if (type !== 'own_dispatch') {
                confirmedOwnDispatchSellers[user_id] = false;
            }

                if(type == 'home_delivery' || type == 'carrier' || type == 'own_dispatch'){
            ...
            if(type == 'carrier'){
                ...
                type_id = $('input[name=dispatcher_id_'+user_id+']:checked').val();
            }
        ...
        if (tradevistaOwnDispatchModalEnabled) {
            $('#ownDispatchConfirm').on('click', function(){
                if (!pendingOwnDispatchSelection) {
                    return;
                }
                confirmedOwnDispatchSellers[pendingOwnDispatchSelection.user_id] = true;
                $('#ownDispatchModal').modal('hide');
                show_pickup_point(pendingOwnDispatchSelection.el, pendingOwnDispatchSelection.user_id);
                pendingOwnDispatchSelection = null;
            });

            $('#ownDispatchDecline').on('click', function(){
                if (!pendingOwnDispatchSelection) {
                    return;
                }
                const userId = pendingOwnDispatchSelection.user_id;
                const fallback = $('input[name="shipping_type_'+userId+'"]').not(pendingOwnDispatchSelection.el).first();
                if (fallback.length) {
                    fallback.prop('checked', true);
                    show_pickup_point(fallback.get(0), userId);
                }
                pendingOwnDispatchSelection = null;
            });
        }
