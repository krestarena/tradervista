                ...
                @if ($order->shipping_type === 'pickup_point')
                    ...
                    <div class="col-12 mt-3">
                        <div class="alert alert-soft-primary d-flex flex-column flex-lg-row justify-content-between align-items-start">
                            <div>
                                <h6 class="mb-1 fw-700">{{ translate('Click & Collect Window') }}</h6>
                                <p class="mb-0 small text-muted">{{ translate('Confirm the pickup ready time and window before switching to Ready for Pickup.') }}</p>
                            </div>
                            <div class="mt-3 mt-lg-0 text-left text-lg-right">
                                <div class="small">{{ translate('Ready at') }}: <strong>{{ optional($order->pickup_ready_at)->format('d M Y, h:i A') ?? translate('Pending') }}</strong></div>
                                <div class="small">{{ translate('Window ends') }}: <strong>{{ optional($order->pickup_window_end)->format('d M Y, h:i A') ?? translate('Pending') }}</strong></div>
                                <div class="small">{{ translate('Payment Protection') }}:
                                    <strong>
                                        @if ($order->payment_protection_status === App\Models\Order::PAYMENT_PROTECTION_RELEASED)
                                            {{ translate('Released') }}
                                        @elseif ($order->payment_protection_status === App\Models\Order::PAYMENT_PROTECTION_ACTIVE)
                                            {{ translate('Active until window closes') }}
                                        @else
                                            {{ translate('Queued') }}
                                        @endif
                                    </strong>
                    ...
                    <div class="col-md-3 mt-3">
                        <label class="form-label" for="pickup_ready_at">{{ translate('Pickup ready timestamp') }}</label>
                        <input type="datetime-local" class="form-control" id="pickup_ready_at" value="{{ $pickupReadyValue }}">
                    </div>
                    <div class="col-md-3 mt-3">
                        <label class="form-label" for="pickup_window_hours">{{ translate('Pickup window (hours)') }}</label>
                        <input type="number" min="1" class="form-control" id="pickup_window_hours" value="{{ $pickupWindowHours }}">
                        <small class="text-muted">{{ translate('Buyers will see this window at checkout and in their orders.') }}</small>
                    </div>
                @endif
            </div>
...
@section('script')
    <script type="text/javascript">
        $('#update_delivery_status').on('change', function() {
            ...
            if ($('#pickup_ready_at').length) {
                payload.pickup_ready_at = $('#pickup_ready_at').val();
                payload.pickup_window_hours = $('#pickup_window_hours').val();
                if (status === 'ready_for_pickup' && (!payload.pickup_ready_at || !payload.pickup_window_hours)) {
                    AIZ.plugins.notify('danger', '{{ translate('Provide a pickup timestamp and window before marking Ready for Pickup.') }}');
                    $('#update_delivery_status').val('{{ $delivery_status }}');
                    AIZ.plugins.bootstrapSelect('refresh');
                    return;
                }
            }
            $.post('{{ route('seller.orders.update_delivery_status') }}', payload, function(data) {
