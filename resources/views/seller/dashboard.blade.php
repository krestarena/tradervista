...
@section('panel_content')
    ...
    @php
        $authUser = auth()->user();
        $kycStatus = $latestKycSubmission->status ?? null;
    @endphp
    <div class="row">
        ...
        <div class="col-sm-6 col-md-6 col-lg-3 mb-4">
            ...
            <div
                ...
                @if ($authUser->shop?->verification_status == 0)
                    @php
                        $kycMessage = [
                            'pending' => translate('Your KYC is under review. You can still update your submission.'),
                            'rejected' => translate('Your previous submission was rejected. Update details to resubmit.'),
                        ][$kycStatus] ?? translate('Submit your KYC details to unlock verified seller benefits.');
                    @endphp
                    <div class="my-n4 py-1 text-center">
                        <img src="{{ static_asset('assets/img/non_verified.png') }}" alt=""
                            class="w-xxl-130px w-90px d-block">
                        <p class="text-muted small mt-3">{{ $kycMessage }}</p>
                ...
                @else
                    <div class="my-2 py-1">
                        ...
                        <p class="text-muted small mt-3">{{ translate('Verified vendors enjoy instant publishing and Payment Protection perks.') }}</p>
                    </div>
                @endif
            </div>
        </div>
