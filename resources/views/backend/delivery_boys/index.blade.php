        ...
        <table class="table aiz-table mb-0">
            <thead>
                <tr>
                    ...
                    <th>{{ translate('KYC Status') }}</th>
                    <th>{{ translate('Service Areas') }}</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @php $status = $delivery_boy->kyc_status ?? \App\Models\DeliveryBoy::KYC_STATUS_DRAFT; @endphp
                        <span class="badge badge-inline badge-@if($status === \App\Models\DeliveryBoy::KYC_STATUS_APPROVED) success @elseif($status === \App\Models\DeliveryBoy::KYC_STATUS_REJECTED) danger @else warning @endif">
                            {{ ucfirst(str_replace('_', ' ', $status)) }}
                        </span>
                        @if($delivery_boy->kyc_rejection_reason)
                            <small class="text-danger d-block">{{ $delivery_boy->kyc_rejection_reason }}</small>
                        @endif
                    </td>
                    <td>
                        @php $cities = $delivery_boy->serviceAreaCityNames(); @endphp
                        @if(!empty($cities))
                            <span class="d-inline-block">{{ implode(', ', $cities) }}</span>
                        @else
                    ...
                    <td>
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-right dropdown-menu-xs">
                                ...
                                @endcan
                                <form action="{{ route('delivery-boy.kyc.approve', $delivery_boy->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="dropdown-item">{{ translate('Approve TradeVista KYC') }}</button>
                                </form>
                                <a href="javascript:void(0);" class="dropdown-item" onclick="showKycRejectModal('{{ route('delivery-boy.kyc.reject', $delivery_boy->id) }}')">
                                    {{ translate('Reject TradeVista KYC') }}
                                </a>
                            </div>
                        </div>
...
<div class="modal fade" id="kycRejectModal">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" id="kycRejectForm">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title h6">{{ translate('Reject dispatcher application') }}</h5>
                    <button type="button" class="close" data-dismiss="modal"></button>
                </div>
                <div class="modal-body">
...
@section('script')
    <script type="text/javascript">
        ...
        function showKycRejectModal(actionUrl) {
            $('#kycRejectForm').attr('action', actionUrl);
            $('#kycRejectForm').find('textarea[name="rejection_reason"]').val('');
            $('#kycRejectModal').modal('show', {backdrop: 'static'});
        }
