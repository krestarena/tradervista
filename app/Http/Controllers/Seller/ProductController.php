...
use App\Services\TradeVista\VendorVerificationService;
...
class ProductController extends Controller
{
    ...
    protected $vendorVerificationService;
    ...
    public function store(ProductRequest $request)
    {
        ...
        if ($this->handleKycPublishingRules($product)) {
            flash(translate('Product has been inserted successfully'))->success();
        } else {
            flash(translate('Product saved as draft until your KYC is approved.'))->warning();
        }
    ...
    {
        $product = Product::findOrFail($request->id);
        if ($request->status == 1 && !$this->vendorVerificationService->canPublishProducts(Auth::user())) {
            return 3;
        }
    ...
    public function duplicate($id)
    {
        ...
        if ($this->handleKycPublishingRules($product_new)) {
            flash(translate('Product has been duplicated successfully'))->success();
        } else {
            flash(translate('Duplicate saved as draft until your KYC is approved.'))->warning();
        }
        return redirect()->route('seller.products');
    }
    ...
    protected function handleKycPublishingRules(Product $product): bool
    {
        return $this->vendorVerificationService->ensureProductDraftForUnverified(Auth::user(), $product);
    }
}
