...
class PurchaseHistoryController extends Controller
{
    ...
    public function purchase_history_details($id, DisputeService $disputeService)
    {
        $order = Order::with(['disputes' => fn($query) => $query->latest()])->findOrFail(decrypt($id));
        ...
        $disputeEligibility = $disputeService->eligibility($order);

        return view('frontend.user.order_details_customer', [
            'order' => $order,
            'tradevistaDisputeEligibility' => $disputeEligibility,
        ]);
    }
