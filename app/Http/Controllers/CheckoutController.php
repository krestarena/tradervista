...
class CheckoutController extends Controller
{
    ...
    public function __construct(VoucherWalletService $voucherWalletService = null)
    {
        $this->voucherWalletService = $voucherWalletService ?: app(VoucherWalletService::class);
    }
    ...
    public function index(Request $request)
    {
        ...
        if ($carts && count($carts) > 0) {
            ...
            return view('frontend.checkout', compact('carts', 'address_id', 'total', 'carrier_list', 'shipping_info', 'voucherContext'));
        }
    ...
    public function store_shipping_info(Request $request)
    {
        ...
        $dispatcherOptions = $this->getDispatcherOptions($deliveryInfo['city_id'] ?? null);

        return view('frontend.delivery_info', compact('carts', 'carrier_list', 'deliveryInfo', 'dispatcherOptions'));
    }
    ...
    public function store_delivery_info(Request $request)
    {
        ...
        if ($carts && count($carts) > 0) {
            foreach ($carts as $key => $cartItem) {
                ...
                $selectedShippingType = $request['shipping_type_' . $product->user_id] ?? 'home_delivery';
                $isOwnDispatch = $selectedShippingType == 'own_dispatch';
                if ($isOwnDispatch) {
                    $selectedShippingType = 'home_delivery';
                    $this->updateOwnDispatchSelection($product->user_id, true);
                } else {
                    $this->updateOwnDispatchSelection($product->user_id, false);
                }

                if (get_setting('shipping_type') != 'carrier_wise_shipping' || $selectedShippingType == 'pickup_point') {
                    if ($selectedShippingType == 'pickup_point') {
                    ...
                    if ($cartItem['shipping_type'] == 'home_delivery' && !$isOwnDispatch) {
            ...
            return view('frontend.payment_select', compact('carts', 'shipping_info', 'total', 'voucherContext'));
        } else {
    ...
    public function applyVoucher(Request $request)
    {
        $user = $request->user();
        if (!$user) {
            flash(translate('Please log in to use your voucher wallet.'))->warning();
            return back();
        }
        ...
        try {
            $cartTotal = $this->calculateCartGrandTotalForUser($user);
            $this->voucherWalletService->reserveForCheckout($user, (float) $request->amount, $cartTotal);
            flash(translate('Voucher amount applied to your order.'))->success();
    ...
    public function removeVoucher(Request $request)
    {
        ...
        if ($user) {
            $this->voucherWalletService->clearReservation($user);
        } else {
            $this->voucherWalletService->clearReservation();
        }

        flash(translate('Voucher selection removed.'))->success();
    ...
    public function order_confirmed()
    {
        ...
        Session::forget('tradevista_own_dispatch');
        Session::forget(VoucherWalletService::SESSION_KEY);
    ...
    public function updateDeliveryInfo(Request $request)
    {
        ...
        if ($shipping_type == 'own_dispatch') {
            $this->updateOwnDispatchSelection((int) $request->user_id, true);
            $shipping_type = 'home_delivery';
        } else {
            $this->updateOwnDispatchSelection((int) $request->user_id, false);
        }
        foreach ($user_carts as $key => $cartItem) {
    ...
    protected function calculateCartGrandTotalForUser(?User $user): float
    {
        if (!$user) {
            return 0;
        }
        ...
        foreach ($carts as $cartItem) {
            ...
            $total += cart_product_price($cartItem, $product, false, false) * $cartItem['quantity'];
            $total += cart_product_tax($cartItem, $product, false) * $cartItem['quantity'];
    ...
    protected function updateOwnDispatchSelection(int $sellerId, bool $selected): void
    {
        $selections = Session::get('tradevista_own_dispatch', []);
        if ($selected) {
            $selections[$sellerId] = true;
        } else {
            unset($selections[$sellerId]);
        }

        if (empty($selections)) {
            Session::forget('tradevista_own_dispatch');
        } else {
            Session::put('tradevista_own_dispatch', $selections);
        }
    }

    protected function isOwnDispatchSelected(int $sellerId): bool
    {
        $selections = Session::get('tradevista_own_dispatch', []);
        return (bool) ($selections[$sellerId] ?? false);
    }
}
