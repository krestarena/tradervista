...
- **S4 Click & Collect** – pickup orders now capture ready timestamps, configurable windows, and a dedicated `ready_for_pickup` delivery status. Admin and seller order views enforce entering the window before activating pickup, show Payment Protection messaging, and automatically release holds (and referral payouts) once pickups are confirmed or the window expires. Buyers see the pickup window and Payment Protection badge in their order details, and pickup confirmations unlock reviews/refunds like delivered orders.

### Dispatcher Stories (D1–D2)
- **Status:** Delivery boys now double as TradeVista dispatchers. The `delivery_boys` table stores document type/number, upload, license, configurable service-area cities, default rates, ETA hours, and KYC timestamps so Admins can approve/reject submissions while audit-logging reasons. Dispatchers submit KYC from their profile page, and admin index pages expose status badges plus approve/reject actions.
- Checkout now lists “TradeVista Dispatchers” whenever approved couriers cover the buyer’s city. Buyers can compare per-dispatcher price/ETA, select one at delivery step, and the order is automatically assigned to that dispatcher user with Payment Protection enabled until delivery confirmation. `Cart` and `order_details` persist the dispatcher_id so delivery dashboards show new jobs immediately.
...
3. **A3 Referral settings** – extend `business_settings` keys (buyer_referral_pct, seller_referral_pct, min_order_amount, voucher_expiry_days, rounding_mode) with audit logging.
   - _Status_: Admins can manage TradeVista referral math from `/admin/tradevista/referral-settings`, which writes into dedicated `BusinessSetting` keys, enforces confirmation, and captures audits (actor, IP, before/after payload) so changes are tracked and only impact future orders.
4. **A4 Disputes** – add `disputes` table referencing orders, storing evidence attachments, decisions, and status. Tie into Payment Protection release logic.
   - _Status_: Buyers can open disputes from their order details within the configurable 48h window; submissions capture reasons + evidence, pause Payment Protection, and surface in the new admin dispute console with under-review / resolve workflows that immediately release or refund funds and notify referral payouts when a decision is posted.
